import { test, BrowserContext, Page } from '@playwright/test';
import { LoginPage } from '../pages/loginPage';
import { KnowledgePage } from '../pages/knowledgePage';
require('dotenv').config();

test.describe('Knowledge Testing', () => {
  let context: BrowserContext;
  let page: Page;
  let loginPage: LoginPage;
  let knowledgePage: KnowledgePage;

  test.beforeAll(async ({ browser }) => {
    context = await browser.newContext();
    page = await context.newPage();

    loginPage = new LoginPage(page);
    knowledgePage = new KnowledgePage(page);

    // Go to login page
    await loginPage.goto();

    // Get credentials from environment variables
    const username = process.env.ODOO_USERNAME!;
    const password = process.env.ODOO_PASSWORD!;

    // Perform login
    await loginPage.login(username, password);

    // Navigate to the Knowledge page
    await knowledgePage.moveToKnowledgePage();

    // Wait for the Knowledge page to load
    await page.waitForTimeout(5000); // Adjust based on UI speed
  });

  test('Verify Knowledge page header is visible', async () => {
    await knowledgePage.verifyHeaderVisibility();
  });

  test.afterAll(async () => {
    // await context.close(); // Closes the browser context after all tests
  });
});
